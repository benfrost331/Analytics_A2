...
#If the tuning has already been run, used the saved results:
if (file.exists("ValidationLoopResults.Rdata"))
{
  load("ValidationLoopResults.Rdata")
} else 
{
  #cCreate a Data Frame to store the different results for each nu:
...
# Indentifying the index of the best nu value (the one that minimises the validation cross-entropy):
...
# Plotting the response surface for a runner targeting 12km/h, showing the predicted probability of bombing over Fluid and Carbs, with separate plots for each Sex:
