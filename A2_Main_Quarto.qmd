...
# Create sequences over our range of Fluid and Carbs values:
...
# Create input matrices as needed by our split-brain network:
Xu_grid <- t(as.matrix(grid_pred[, c("AveSpeed", "SexNum")]))
Xv_grid <- t(as.matrix(grid_pred[, c("Fluid", "Carbs")]))

# Evaluate the fitted model for each input in the grid:
...
fit_standard_network <- function(X_train, Y_train, m1 = 8, m2 = 2, nu = 0,
                                 maxit = 1000, n_starts = 8, seed = 2026) {
  set.seed(seed)
